...
export default function ReviewPhotoEvalView(props: SeaLifePhotoEvalViewProps) {
    ...
    useEffect(() => {
        if (props.photoRecord?.classification) {
            const animalOption: Option = {
                key: props.photoRecord.classification,
                label: props.photoRecord.classification
            };
            reset({
                ...props.values,
                animal: animalOption
    ...
    }, [props.photoRecord, props.values, reset]);
    ...
    const onSubmit = (data: Form) => {
        if (buttonPressed === 1 && props.photoRecord) props.okPhoto(props.photoRecord.id);
        if (buttonPressed === 2 && props.photoRecord) props.rejectPhoto(props.photoRecord.id);
        if (buttonPressed === 3 && props.photoRecord) props.headerPromote(props.photoRecord, props.photoRecord.divesite_id, props.photoRecord.photoPath);
        if (buttonPressed === 4 && props.photoRecord && props.diveSiteInfo) props.sightingPromote(props.photoRecord, props.diveSiteInfo, data.animal);
    };
    
    return (
        <form onSubmit={handleSubmit(onSubmit, handleError)} className="cols col-12 mt-2 flex-column full-height">
            <img src={`${cloudflareBucketUrl}${photoName}`} width={'60%'} height={'50%'} style={{marginTop: '5%'}} alt="Review" />
    ...
    render={({ field: { onChange, value } }) => {
        console.log("Current value being passed to DynamicSelect:", value);
        
        return (
            <DynamicSelect
                key={value ? value.key : 'empty'}
                value={value}
                onChange={onChange}
                allowCreate={true}
                labelInValue={true}
...
/>
            ...
            <div className="cols col-12 mt-8 flex-row-between">
                <div className="col-1"/>
                <div className="col-2">
                    <Button onClick={() => setButtonPressed(1)} className="btn-md bg-primary" type="submit" disabled={isSubmitting}>Approve</Button>
                </div>
                <div className="col-2">
                    <Button onClick={() => setButtonPressed(2)} className="btn-md" type="submit">Reject</Button>
                </div>
                <div className="col-2">
                    <Button onClick={() => setButtonPressed(3)} className="btn-md bg-primary" type="submit">Dive Site Header</Button>
                </div>
                <div className="col-2">
                    <Button onClick={() => setButtonPressed(4)} className="btn-md bg-primary" type="submit">Sea Life Sighting</Button>
                </div>
                <div className="col-1"/>
            </div>
        </form>
